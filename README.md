...
> 当前版本：`1.2.0` 候选，目标通过 npm `latest` 与正式 GitHub Release 发布。npm 包名：`dsh-codex-community`。
...
- 适配 DeepSeek Harness `0.1.2-rc.1` 的 settings service API，使用 `ctx.settings.installSection` 注册 `dsh-codex` 命名空间；
- 发布包扫描全部 `dist/**`，避免 internal chunk 残留旧的 `installSettingsSection` / `settingsNamespace` 导入；
- Web client inject 改为 `dsh-client-ui-renderer`，不再依赖已停止发布的 `dsh-client-runtime`。

`1.1.2` 维护更新：

- 更新 Cordis `4.0.2`、Schemastery `3.18.2`、pi-ai `0.84.4` 与 TypeScript `7.0.2`，并显式固定 Harness 运行时所需的辅助依赖；
- 启用严格 peer dependency 安装检查，让不兼容的运行时组合在构建与 CI 阶段直接失败；
- 发布候选的 npm 漏洞审计对临时 Registry/网络故障执行有界重试和单次超时；真实漏洞与重试耗尽仍失败关闭。
...
- `1.2.0` 的完整检查与 Linux、macOS、Windows CI/profile smoke 将按本版本候选记录；发布后真实账号验证从 `0/13` 开始，状态见[验收记录](docs/releases/v1.2.0.acceptance.json)；
