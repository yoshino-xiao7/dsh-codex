...
- 最小权限、Action SHA 固定、一次构建、离线确定性 SBOM、首次发布一次性引导、npm Trusted Publisher 切换和安全重跑的发布流程。
- 幂等、离线且保护人工内容的 `release:prepare` 命令，用于生成后续版本双语发布骨架和全新验收记录，并在任何冲突或受管路径符号链接存在时于写入前失败。
- 不联网、不读取凭据的 `release:acceptance` 离线证据记录器：只在明确列齐固定断言后记录通过，把全部证据绑定到同一候选 SHA，状态输出不显示证据值，提交存在性/祖先关系仍由严格门禁验证，审批仍由维护者人工作出。
...
- 真实 OAuth 和真实 Codex 网络 smoke：公开 Release 前必须补齐受控账号证据
...
- a least-privilege release flow with pinned Action SHAs, one artifact build, an offline deterministic SBOM, a one-time first-publish bootstrap, migration to npm Trusted Publisher, and safe reruns.
...
- an offline `release:acceptance` evidence recorder that neither accesses the network nor reads credentials, records a pass only when every fixed assertion is explicitly listed, binds all evidence to one candidate SHA, hides evidence values from status output, leaves commit existence/ancestry checks to the strict gate, and leaves approval to a maintainer.
...
- Real OAuth and real Codex network smoke require controlled-account evidence before the public Release
