...
- 不发布、不读取凭据的候选复演命令 `pnpm run release:candidate -- 0.0.1`：要求已提交且工作树干净，并固定 Node 24、pnpm `10.34.5` 与 npm `11.16.0`；它复演 frozen install、完整检查、仅一次生成候选 tarball、dry-run、SBOM、DSH profile smoke、隔离导入与生产依赖 audit，写入 `release/`，已有输出时在写前失败。权威候选仍由 `main` 上的发布 workflow 生成，macOS/Windows 本地复演不替代 Linux x64 发布证据。
...
- a credential-free, non-publishing candidate replay command, `pnpm run release:candidate -- 0.0.1`. It requires a committed checkout with a clean working tree and pins Node 24, pnpm `10.34.5`, and npm `11.16.0`; it replays frozen installs, the complete checks, exactly one artifact-producing pack, dry-run, SBOM, DSH profile smoke, isolated import, and production-dependency audit into `release/`, failing before any write when output already exists. The authoritative candidate still comes from the release workflow on `main`; a macOS or Windows local replay does not replace Linux x64 publication evidence.
