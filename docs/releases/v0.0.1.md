...
跨平台与真实账号验收记录保存在 [`v0.0.1.acceptance.json`](v0.0.1.acceptance.json)。平台 CI/profile smoke 必须先由维护者核验，再用离线 `pass-platform` 记录；该命令不联网、不检查提交存在性，也不能覆盖不同证据。平台与真实网络证据必须绑定同一 SHA。真实验收必须逐项证明 Web OAuth、模型目录、文本/reasoning 流、终态 usage、安全工具闭环、两轮 replay、`maxPixels=4194304` 图片路径、四种 transport 与 Fast，不能用自由文本概括替代固定断言。记录只能包含测试时间、环境、布尔结果和证据链接，不能包含 token、OAuth code、Cookie 或账号凭据。所有项目通过并由维护者批准前，严格发布门禁保持关闭。
