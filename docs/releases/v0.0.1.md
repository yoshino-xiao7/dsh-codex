...
- 修复 Linux 对 PR 模板文件名大小写敏感，以及 Windows CRLF 文本与 `pnpm.cmd` 启动方式导致跨平台维护门禁误失败；
...
- Fix cross-platform maintenance gates that failed on Linux because of pull-request-template filename casing and on Windows because of CRLF text and `pnpm.cmd` invocation semantics.
