...
`1.1.0` 是当前正式版。完整检查与 Linux、macOS、Windows CI/profile smoke 已按本版本候选 `3/3` 通过并获维护者批准；发布后真实账号验证从 `0/13` 开始，不继承 `1.0.0` 的候选、平台或真实账号证据。
...
| npm / GitHub Release | 严格工作流校验候选、Registry 回读、provenance、签名与 Release 资产 | 正式版 [`v1.1.0`](https://github.com/yoshino-xiao7/dsh-codex/releases/tag/v1.1.0) |

`1.1.0` 新增 Fast、Transport、回复详略和推理摘要的持久化全局默认值；会话只保存显式覆盖，Transport 默认变化只切换继承会话的连接 generation。它还提供不轮询的低额度与重置提醒、需二次确认的一次性真实网络诊断，以及最多 20 条、不落盘的脱敏诊断历史。本机和账号诊断仍不发送模型请求；真实网络诊断会消耗额度，只使用一个经校验的模型、SSE、关闭 Fast 的短提示且不自动重试。自动化与三平台证据已绑定本版本候选；真实账号结果继续在发布后验收中记录。

`1.1.0` 按正式版发布，同时只声明精确的 DSH 预发布依赖目标。候选提交、Linux/macOS/Windows CI/profile smoke 与维护者批准记录在本版本[验收记录](releases/v1.1.0.acceptance.json)中；真实账号验证以 `0/13` 如实发布并在发布后继续记录。上一版的[验收记录](releases/v1.0.0.acceptance.json)仅作为历史证据保留，不能替代本版本门禁。
...
`1.1.0` 的 bundle 不修改通用 `llm-pi-ai` Cordis row。它提供：
...
- 请通过插件设置页或 `/codex-login cancel` 取消登录：写入线性化前会正常取消，提交已经开始时会明确提示无法取消并继续等待最终状态；`logout` 始终删除凭据。DSH 当前公共 seam 没有“提交后禁止取消”能力，第三方代码直接调用 `ctx.authorization.cancel()` 会绕过这一协调，不属于 `1.1.0` 支持的交互路径；
