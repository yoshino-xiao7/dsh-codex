...
`1.2.1` 是当前正式版。完整检查与 Linux、macOS、Windows CI/profile smoke 已按本版本候选 `3/3` 通过并获维护者批准；发布后真实账号验证从 `0/13` 开始，不继承旧版本证据。
...
| npm / GitHub Release | 严格工作流校验候选、Registry 回读、provenance、签名与 Release 资产 | 正式版 `v1.2.1` 由本次受保护工作流生成 |

`1.2.1` 保留 `1.2.0` 的运行时兼容与恢复语义，并为 Web client 注入 `remote.llm` / `remote.settings`，通过 Harness `0.1.2-rc.1` typed remote 合同恢复模型管理，同时保留旧版连接接口回退。

`1.2.1` 只声明精确的 DSH 预发布依赖目标。候选提交、Linux/macOS/Windows CI/profile smoke 与维护者批准记录在本版本[验收记录](releases/v1.2.1.acceptance.json)中；真实账号验证从 `0/13` 独立开始。`1.2.0` 与更早版本的记录仅作为历史证据保留，不能替代本版本门禁。
...
`1.2.1` 的 bundle 不修改通用 `llm-pi-ai` Cordis row。它提供：
...
- 请通过插件设置页或 `/codex-login cancel` 取消登录：写入线性化前会正常取消，提交已经开始时会明确提示无法取消并继续等待最终状态；`logout` 始终删除凭据。DSH 当前公共 seam 没有“提交后禁止取消”能力，第三方代码直接调用 `ctx.authorization.cancel()` 会绕过这一协调，不属于 `1.2.1` 支持的交互路径；
