...
| Node.js | 本地 `22.22.2` 完整测试；0.0.2 三平台候选运行证据为 `TBD` | `>=22.19.0 <25`；候选运行待完成 |
| macOS arm64 | 本地测试、构建、attachment seam 与隔离 profile 启动；0.0.2 平台运行证据为 `TBD` | 本地自动化已验证；平台门禁 `pending` |
| Windows x64 | 0.0.2 `windows-latest` Node 22/24 候选运行证据为 `TBD` | 平台门禁与真实用户环境 `pending` |
| Linux x64 | 0.0.2 `ubuntu-latest` Node 22/24 候选运行证据为 `TBD` | 平台门禁 `pending` |
...
| npm / GitHub Release | 严格工作流将校验候选、Registry 回读与 Release 资产；0.0.2 发布证据为 `TBD` | 正式发布 `pending` |

`0.0.x` 是技术预览，只对表中的精确 DSH 预发布版本声明兼容。`0.0.2` 当前三平台候选运行证据均为 `TBD / pending`；正式发布前必须完成 Linux、macOS 和 Windows CI/profile smoke `3/3`、完整供应链校验和维护者批准。真实 OAuth、对话、图片、transport 和 Fast 网络验收当前为 `0/13 pending`，正式发布后再逐项补齐；这些未完成项必须如实展示，不能冒充已验证能力。额度窗口读取已独立完成本机设置页验证，不计入这 13 个会消耗模型请求或需要完整交互闭环的验收项。
