...
1. 确认运行的是已安装的精确包体；当前正式版为 `dsh-codex-community@1.0.0`，`1.1.0` 仍在开发，不要把不同版本的文件混入同一 profile；
...
设置页每次进入、页面重新可见、窗口重置节点以及手动点击“刷新”时，会由 Host 请求真实的五小时与每周使用窗口；每周相对天数递减和进入不足 24 小时区间都只切换本地显示，不额外联网。`/codex-usage refresh` 使用相同 Reader 主动读取账号窗口，`/codex-usage status` 则只查看最近请求产生的脱敏 `QuotaObserver` 观测，用于实时读取不可用时的安全降级。failure 中的 reset 必须通过严格格式和有限时距校验，否则耗尽观测会在有限时间后回到“未知”。
...
`1.1.0` 的提醒只依据已验证的 Codex 5 小时和每周窗口，剩余不高于 20% 时才显示；安全读数为“未知”、读到的是其他类型窗口或设置页已关闭时，都不会推测提醒。这是插件页面内提示，不是系统通知，也没有常驻后台轮询。

到达已验证的重置时间时，页面只会自动刷新一次。若本次确认失败，会保留上一次安全数据并请用户手动刷新，不自动循环重试。
...
先确认当前选择的是 GPT-5.4、GPT-5.5、GPT-5.6 Luna、Sol 或 Terra；闪电按钮在 GPT-5.3 Codex Spark、GPT-5.4 mini 和未知模型上不可用，也不会发送 Fast service tier。点击模型选择器左侧的闪电按钮，或运行 `/codex status` 与 `/codex set fast on|off` 检查和修改当前会话状态。切换从下一次请求生效；`1.1.0` 中的进程重启会清除当前会话覆盖，然后使用插件设置中持久化的 Fast 全局默认值。

Fast 使用官方 priority service tier，目标速度为 1.5 倍并消耗更多额度。插件不会把失败请求自动降级重放；关闭 Fast 后，需要由用户决定是否重新发送请求。

## 全局默认值没有生效

设置页中的 Fast、Transport、回复详略和推理摘要是全局默认值，不会覆盖当前会话已明确选择的字段。要让当前会话重新继承全局值，运行 `/codex reset`。这会删除该会话的四项显式覆盖，不会删除全局设置。

全局 Transport 更改只让当前继承该默认值的会话切换到新的 transport generation；显式覆盖 Transport 的会话保持不变。下一次请求不会继承旧连接、缓存或 SSE 回退锁存；已经进行的请求继续持有旧 generation，等 stream 与 iterator teardown 收敛后再清理。
...
- “真实网络诊断”会先显示消耗说明和本次实际验证的模型 ID；只有用户再次确认后，才会用同一模型通过 SSE、关闭 Fast 发送一条固定短提示。请求遵循 Codex SDK 的公开 wire schema，不携带会话历史或工具，也不自动重试；专用端点没有可用的服务端硬输出上限，插件会在首次可见文本后立即结束流，但取消或结束流不能保证未消耗额度。自动化只覆盖 mock HTTP 边界，线上专用端点仍需真实账号验收。

诊断只显示固定检查项、状态码、受限事实和经校验的实际模型 ID，不显示 token、account ID、请求 ID、提示、模型输出、原始响应或原始错误。最多 20 条历史只存在当前进程，重启后清空；读取、导出和清空都需显式操作，导出的 JSON 与页面展示使用同一安全 schema。真实网络诊断只能证明这一次 SSE 短文本请求的结果；它没有服务端硬输出上限，只在首次可见文本后尽快结束流，不能替代图片、工具、Fast 或其他 Transport 的验收。

真实网络诊断的本机状态与凭据前置检查最多等待 30 秒；显示 `preflight-timeout` 时尚未进入模型请求 probe，可排查本机状态或凭据服务后重试。若持续显示 `model-request-busy`，表示旧请求的 iterator teardown 或临时资源清理无法确认；页面重载不会解除进程级隔离，请完整退出并重启应用进程后再尝试，避免重复消耗额度。
