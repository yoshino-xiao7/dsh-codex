...
The `0.0.x` line is a technical preview and declares compatibility only with the exact DSH release candidate above. Before `0.0.1` is formally published, Linux, macOS, and Windows CI/profile smoke must reach `3/3`, supply-chain verification must be complete, and a maintainer must approve the release. The real OAuth, conversation, image, transport, and Fast rows may remain at `0/13` when the formal release is published and are completed one by one afterward. Incomplete checks must remain visible and must never be presented as verified capabilities; findings are fixed and iterated in `0.0.2`.
