...
`1.2.1` is the current stable release. Complete checks and Linux, macOS, and Windows CI/profile smoke passed `3/3` against this candidate and were approved by the maintainer. Post-release live-account validation starts at `0/13` and inherits no older evidence.

| Component or environment | `1.2.1` compatibility target | Status |
| --- | --- | --- |
...
| npm / GitHub Release | The strict workflow verifies the candidate, Registry readback, provenance, signatures, and Release assets | Stable `v1.2.1` is created by this protected workflow |

`1.2.1` preserves the runtime compatibility and recovery semantics from `1.2.0`. It injects `remote.llm` / `remote.settings` into the Web client and restores model management through the Harness `0.1.2-rc.1` typed-remote contracts while retaining the legacy connection fallback.

`1.2.1` declares only the exact DSH prerelease dependency target. Its candidate commit, Linux/macOS/Windows CI/profile smoke, and maintainer approval are recorded in this version's [acceptance record](releases/v1.2.1.acceptance.json). Live-account validation starts independently at `0/13`. Records for `1.2.0` and older releases remain historical evidence only and do not replace this version's gates.
...
The `1.2.1` bundle does not modify the general `llm-pi-ai` Cordis row. It provides:
...
- `1.2.1` does not provide image generation or editing.
