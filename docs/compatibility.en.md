...
| Node.js | Complete local suite on Node 22; candidate CI will cover Node 22/24 | `>=22.19.0 <25`; three-platform candidate pending |
| macOS | `macos-latest` Node 22/24 complete checks, frozen DSH installation, and Web/profile smoke | `0.0.3` platform gate pending |
| Windows x64 | `windows-latest` Node 22/24 complete checks, frozen DSH installation, and Web/profile smoke | `0.0.3` platform gate pending; real user environment pending |
| Linux x64 | `ubuntu-latest` Node 22/24 complete checks, frozen DSH installation, and Web/profile smoke | `0.0.3` platform gate pending |
...
| npm / GitHub Release | The strict workflow will verify the `0.0.3` candidate, Registry readback, provenance, signatures, and Release assets; the latest published artifacts remain [`v0.0.2`](https://github.com/yoshino-xiao7/dsh-codex/releases/tag/v0.0.2) | Formal `0.0.3` publication awaits three-platform acceptance and maintainer approval |

The `0.0.x` line is a technical preview and declares compatibility only with the exact DSH release candidate above. Linux, macOS, and Windows CI/profile smoke for `0.0.3` is currently `0/3 pending` and must reach `3/3` before publication. The candidate commit, exact Node versions, and job links will be recorded in the [acceptance record](releases/v0.0.3.acceptance.json) with maintainer approval. Live OAuth, conversation, image, transport, and Fast network acceptance starts in a fresh `0/13 pending` record and will be completed individually post-release. These incomplete checks must remain visible and must never be presented as verified capabilities. Usage-window and model-setting behavior was verified independently in the local Chinese and English settings page and is not one of these 13 checks that consume a model request or require a complete interaction round trip.
