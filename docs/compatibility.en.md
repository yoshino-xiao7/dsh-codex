...
| Node.js | Complete local suite on `22.22.2`; CI covers 22 and 24 | `>=22.19.0 <25` |
| macOS arm64 | Local tests, build, attachment seam, and isolated profile startup | Partially verified |
| Windows x64 | This package's source path contains no native module | CI and device pending |
| Linux x64 | This package's source path contains no native module | CI pending |
