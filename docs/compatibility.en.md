...
| Node.js | Complete local suite on `22.22.2`; 0.0.2 three-platform candidate-run evidence is `TBD` | `>=22.19.0 <25`; candidate runs pending |
| macOS arm64 | Local tests, build, attachment seam, and isolated profile startup; 0.0.2 platform-run evidence is `TBD` | Local automation verified; platform gate `pending` |
| Windows x64 | 0.0.2 `windows-latest` Node 22/24 candidate-run evidence is `TBD` | Platform gate and real user environment `pending` |
| Linux x64 | 0.0.2 `ubuntu-latest` Node 22/24 candidate-run evidence is `TBD` | Platform gate `pending` |
...
| npm / GitHub Release | The strict workflow will verify the candidate, Registry readback, and Release assets; 0.0.2 publication evidence is `TBD` | Formal publication `pending` |

The `0.0.x` line is a technical preview and declares compatibility only with the exact DSH release candidate above. All three 0.0.2 platform candidate-run records are currently `TBD / pending`; Linux, macOS, and Windows CI/profile smoke must reach `3/3`, supply-chain verification must be complete, and a maintainer must approve the formal release. Live OAuth, conversation, image, transport, and Fast network acceptance is currently `0/13 pending` and will be completed individually after publication. These incomplete checks must remain visible and must never be presented as verified capabilities. Reading the usage windows was verified independently in the local settings page and is not one of these 13 checks that consume a model request or require a complete interaction round trip.
