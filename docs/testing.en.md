...
- `codex-session-resources.test.mjs`: namespaced sessions, reset/agent/runtime cleanup, sanitized transport-health counters, and isolation from an external in-process pi-ai session;
...
- `codex-connection-diagnostics.test.mjs`: zero account reads in local mode, exactly one usage read in account mode, fixed classifications for 401/403, 429, 5xx, and other HTTP failures, cancellation, fixed reports, loopback-only RPC, and no token/account-ID/raw-error leakage; the module has no stream seam;
...
- `codex-pi-provider.test.mjs`: default payload, per-session Fast, transport, reply verbosity, reasoning summary, and no automatic downgrade;
- `session-preferences.test.mjs`, `session-preference-command.test.mjs`, and `session-preference-bridge.test.mjs`: session isolation, capacity bounds, reset, Fast-support queries, all four Transport choices, reply verbosity, reasoning summary, sanitized transport health, and command/RPC error boundaries;
...
- `client-bundle.test.mjs`: Web sign-in with explicit OAuth link/code copy, plan and usage presentation, visibility/reset-boundary active refresh plus the local 24-hour display transition, generic discovery plus the independent capability RPC, no unknown-capability inference, model-enablement writes, Fast/Transport/verbosity/summary controls, sanitized transport health, collapsed diagnostics with safe report copying, mutually exclusive sign-in actions, unmount safety, multi-instance/hot-reload style lifecycle, narrow-screen layout, and loopback RPC boundaries;
